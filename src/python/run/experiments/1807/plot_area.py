...
for model in models:
    ...
    for iou_thresh in [0.4]:
        for min_box_area in [0.001, 0.025, 0.05, 0.1, 0.25]:
            results = load_file(
                'out/1807/' + model + '/test/test_boxrangeflight_iou{}-area{}_result_metric.pkl'.format(iou_thresh,
                                                                                                        min_box_area))
...
pr_total = BaseMultiPlot(x_data=[[0.001, 0.025, 0.05, 0.1, 0.25]] * len(total_precisions),
