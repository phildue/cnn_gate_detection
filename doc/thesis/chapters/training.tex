...
\Cref{fig:pp_bar} shows the results in terms of average precision in the training domain. On the simulated data the models with image augmentation general perform a bit better than without the augmentation. Between the models using augmentation there is no method that clearly performs better.

On the real data two methods lead to significant improvement. Especially blurring improves the detection. In contrast image augmentation with chromatic aberration or variations in exposure leads to worse results than using no image augmentation at all.
...
In contrast on the real data some methods lead to a deterioration in performance, while others improve the detection. The scaling in HSV space improves the detection. Hence, it can be assumed that there was not sufficient variation of colours in the training domain. Also blurring improves the detection. This was a pattern also observed visually and incorporating it in the training domain helped the detection.

Modelling other sensor effects such as variation in exposure and chromatic aberration made the detection worse. Hence it seems that these patterns are not present in the test domain.
\fi
\subsection{Conclusion}
