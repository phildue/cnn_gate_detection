...
for i, model in enumerate(models):
    conf_thresh = 0
    summary = load_file('out/2606/' + model + '/summary.pkl')
    architecture = summary['architecture']
    grid = grids[i]
    pred = GateNet.create_by_arch(norm=img_res[i], architecture=architecture,
                                  ...
                                  augmenter=None,
                                  weight_file='out/2606/' + model + '/model.h5'
                                  )
    for iou in ious:
        evaluate_file(pred, 'out/2606/' + model + '/results/result_0.pkl',
