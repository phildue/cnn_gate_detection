...
\end{enumerate}
	...
	In total this leads to 5 predicted parameters for each bounding box and thus to a mapping from the input image of 416x416x3 pixels to 12675 output nodes that predict 2535 boxes. A final reshape operation leads to a predicted matrix of dimension (2535,5), where each line corresponds to a predicted box. In a last step boxes that contain the same class and have a high overlap are filtered such that only the boxes with the highest confidences remain.	
	...
	In order to train a \ac{CNN} to predict the desired properties a ground truth has to be defined for each of the 12675 output nodes. Subsequently the loss is formulated as derivable function and the \ac{CNN} can be trained with the backpropagation.
	
	A ground truth label can simply be encoded by inverting the equations in \Cref{sec:encoding}. Yet the question remains to which grid cell and anchor box a label is assigned to. Therefore the \ac{IoU} between every ground truth and anchor box is calculated and the grid with the highest value is assigned. If a ground truth box has very different coordinates than any possible anchor box, even the highest \ac{IoU} is comparatively low. Thus the network has to perform a very strong activation to predict such a label. In return the gradient will be high which can cause instable updates. Therefore labels that have a lower \ac{IoU} than 0.5 are excluded. 
	
	Hence the ground truth for one image is defined as a matrix with dimension (2535,5) where each line corresponds to one anchor at one grid cell. Anchors that have assigned a ground truth object contain the respective coordinate offsets while other anchors are zero.
...
\end{figure}
	...
	The experiments show how the performance of the network drops for larger objects while even a few layers are enough for small objects. This is the opposite as reported for example by \todoref{Objectdetect}. We hypothesize that this is due to the emptiness of \ac{EWFO}. As the object gets larger, the features are spread further and further a part while more and more noise is present in the receptive field of the detector.
