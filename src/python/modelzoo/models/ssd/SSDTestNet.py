...
from modelzoo.models.ssd import SSDNet
...
class SSDTestNet(SSDNet):
    ...
    def build_model(self):
        ...
        with K.name_scope('ClassPredictors'):
            pred_c, predictor_sizes = self._build_class_predictors(conv7, conv6, conv4)

        with K.name_scope('LocalizationPredictors'):
            pred_loc = self._build_loc_predictors(conv7, conv6, conv4)
    ...
    def _build_class_predictors(self, conv7, conv6, conv4):
        ...
        return Activation('softmax', name='classes_softmax')(classes_concat), predictor_sizes
