    ...
    for it in range(n_iterations):
        ...
        try:
            ...
            for iou in [0.4, 0.6, 0.8]:
                ...
                frame['{2:s}_r{0:02f}_i{1:02d}'.format(iou, it, dataset)] = recalls
                frame['{2:s}_p{0:02f}_i{1:02d}'.format(iou, it, dataset)] = precisions
                print(frame.to_string())
                frame.to_pickle('{}/results_total_pr.pkl'.format(prediction_dir))
                frame.to_excel('{}/results_total_pr.xlsx'.format(prediction_dir))
