...
from evaluation.evaluation import evalcluster_yaw_dist
...
for iou in ious:
    ...
    for i, f in enumerate(models):
        result_file = load_file('out/{0:s}_i{1:02d}/test_{2:s}/predictions.pkl'.format(f, 0, dataset))
        labels_pred = result_file['labels_pred']
        labels_true = result_file['labels_true']
        img_files = result_file['image_files']
        fn, tp = evalcluster_yaw_dist(labels_true, labels_pred, conf_thresh=0.1, n_bins_angle=angle_bins,
...
for i, r in enumerate(recalls):
    plt.subplot(1, len(legend), i + 1)
    plt.pcolor(r, cmap=plt.cm.viridis, vmin=0, vmax=1.0)
    if i < 3:
        plt.title(legend[i], fontsize=12)
...
for i, tp in enumerate(tps):
    ...
    for j in range(len(tp)):
        ...
        print("{}: IoU={} Total {}/{} [{}]: r={}".format(legend[j], ious[i], tp_total, fn_total, tp_total + fn_total,
