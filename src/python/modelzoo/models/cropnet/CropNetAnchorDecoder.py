...
class CropNetAnchorDecoder(Decoder):
    ...
    def decode_coord(self, coord_t):
        ...
        coord_t_dec[:, 0] *= coord_t_dec[:, -1]
        coord_t_dec[:, 1] *= coord_t_dec[:, -1]
        coord_t_dec[:, 2] *= coord_t_dec[:, -1]

        coord_t_dec[:, 0] += coord_t_dec[:, -3]
        coord_t_dec[:, 1] += coord_t_dec[:, -2]
        coord_t_dec[:, 1] = self.norm[0] - coord_t_dec[:, 1]

        return np.hstack((coord_t_dec[:, :3], coord_t_dec[:, 2:3]))
