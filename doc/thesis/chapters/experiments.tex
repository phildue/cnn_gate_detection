...
Due to the challenges in this dataset we hypothesize that the detector trained so far will perform poorly on this dataset. In order to handle overlapping objects in difficult angles, such situations should be included in the training set. A straightforward way is placing the camera randomly (within some margin) in order to cover a large variation of views on the object. That way the network should learn a general object representation and detect unseen objects from different view points. However, random  placement might not resemble the real world sufficiently. An \ac{MAV} does not appear at random places within a scene, especially not when it follows a racing track. Alternatively, the samples can be generated by simulating a flight through a race court. Although this requires to create a race court an a corresponding trajectory the obtained samples should resemble the real world better. 
...
The view angles obtained when simulating a flight strongly depend on the racing track as well as the trajectory the camera flies. Hence, samples obtained with random placement are more general than when simulating a flight. However, it is possible that too many view points in the training set confuse the detector. In that case it would be beneficial to reduce the amount of view angles, ideally by analysing the flight behaviour of an \ac{MAV}. 

We investigate how the detector deals with more view points in training and test set.
