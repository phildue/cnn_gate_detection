...
\end{enumerate}
	...
	\acp{CNN} such as the \textit{TinyYoloV3}-network combine simple features to a more complex image representation layer by layer. Thereby pooling reduces the spatial operation and removes irrelevant data while task relevant information is propagated in the deeper layers. Hence, in deeper layers information that was spread over large parts of the image gets combined and interpreted. We hypothesize that this will lead to a problem when detecting \acp{EWFO} with a \acp{CNN}. The features are sparse and spread over large parts of the image while a majority of the object contains background or even other objects. Features that appear within that part can activate the network although the information should be discarded. Hence, the detector can be distracted by the objects that appear within the empty part.
	
	If this is true it can be expected that the performance of the network decreases as features are spread over larger parts of the image. Hence, we hypothesize that the performance decreases for larger objects. On the other hand \ac{EWFO} also have thin structures. Therefore, the performance should also decrease for very small objects. This contradicts the findings of \todoref{object detect tensorflow} which reports a performance increase with object size. Yet
