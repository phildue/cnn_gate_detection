...
for n_i, n in enumerate([1, 2, 3]):
    for i in range(iterations[n_i]):
        ...
        architecture = [
            ...
            {'name': 'max_pool', 'size': (2, 2)},
            {'name': 'conv_leaky', 'kernel_size': (3, 3), 'filters': int(128 / 2 ** n), 'strides': (1, 1),
             'alpha': 0.1},
            {'name': 'max_pool', 'size': (2, 2)},
            {'name': 'conv_leaky', 'kernel_size': (3, 3), 'filters': int(256 / 2 ** n), 'strides': (1, 1),
             'alpha': 0.1},
            {'name': 'max_pool', 'size': (2, 2)},
            {'name': 'conv_leaky', 'kernel_size': (3, 3), 'filters': int(512 / 2 ** n), 'strides': (1, 1),
             'alpha': 0.1},
            {'name': 'max_pool', 'size': (2, 2)},
            {'name': 'conv_leaky', 'kernel_size': (3, 3), 'filters': int(1024 / 2 ** n), 'strides': (1, 1),
             'alpha': 0.1},
            {'name': 'max_pool', 'size': (2, 2)},
            {'name': 'conv_leaky', 'kernel_size': (1, 1), 'filters': int(256 / 2 ** n), 'strides': (1, 1),
             'alpha': 0.1},
            {'name': 'conv_leaky', 'kernel_size': (3, 3), 'filters': int(512 / 2 ** n), 'strides': (1, 1),
             'alpha': 0.1},
            {'name': 'predict'},
            {'name': 'route', 'index': [11]},
            {'name': 'conv_leaky', 'kernel_size': (1, 1), 'filters': int(128 / 2 ** n), 'strides': (1, 1),
             'alpha': 0.1},
        ...
        def filter(label):
            objs_in_size = [obj for obj in label.objects if
                            min_obj_size < (obj.poly.height * obj.poly.width) / (
                                    img_res[0] * img_res[1]) < max_obj_size]
