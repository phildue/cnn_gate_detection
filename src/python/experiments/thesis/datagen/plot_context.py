...
for m, model in enumerate(models):
    ...
    meanAp = np.mean(m_p)
    errAp = np.mean(std_p)
    results_on_sim.append(meanAp)#, errAp
    plt.plot(m_r, m_p, 'x--')
frame['Sim Data'] = pd.Series(results_on_sim)
...
for m, model in enumerate(models):
    ...
    results_on_real.append(np.mean(meanAp))#, np.mean(np.mean(err_p, 0))

frame['Real Data'] = pd.Series(results_on_real)
