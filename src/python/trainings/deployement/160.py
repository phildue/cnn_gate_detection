...
for i in range(1, 2):
    ...
    def filter(label):
        objs_in_size = [obj for obj in label.objects if
                        min_obj_size < (obj.poly.height * obj.poly.width) / (img_res_data[0] * img_res_data[1]) < max_obj_size]
    ...
    model, output_grids = build_detector(img_shape=(img_res_network[0], img_res_network[1], 3), architecture=architecture,
                                         anchors=anchors,
                                         n_polygon=4)
