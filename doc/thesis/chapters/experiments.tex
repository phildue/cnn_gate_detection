...
\begin{itemize}
	\item \textit{Gray} is trained by transforming 50\% of the images to grayscale images during training. This should force the network to learn more colour invariant features and thus improve the results on the \textit{Hallway} dataset. 
...
Several image augmentation techniques are applied in order to bridge this gap. Thereby \textit{distortion} and \textit{grey} lead to an improvement. With \textit{grey} affecting the colour feature this would mean the network trained without augmentation overfitted to the colour in simulation. If that is the case the network trained with 

One difference is the colour. Especially on the \textit{Hallway} data set the objects do not appear orange. In contrast in \textit{Cyberzoo} the colour is very distinctive. Image augmentation techniques that mainly address this feature are variations in \ac{HSV}, chromatic aberration and including grey images. If the network overfits to the colour in the simulated set one would expect that any of these methods leads to an

Overall the best results are achieved in \textit{Cyberzoo}. This is not surprising as there the light conditions are relatively easy. The background is black and the object is clearly visible. With the colour being a significant feature in this data set, it is even more surprising that the network trained with \textit{Gray} augmentation here achieves the highest results.

\textit{Basement} has more difficult light conditions and more overlapping objects. Compared to that the performance difference to \textit{Cyberzoo} is relatively low. In \textit{Hallway} the light conditions are very difficult as the objects stand in front of the window. The detector seems to struggle with this too. Several networks achieve an average precision of 0. It seems that despite having similar examples in the training set, the detector can not transfer the results to the real world.
