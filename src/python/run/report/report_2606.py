...
def load_val_acc(netname, img_res, grid, layers, filters, old=True):
    if old:
        if filters == 16:
    ...
    content = load_file('out/' + folder_name + '/log.csv')
    val_acc = float(max([c[4] for c in content[1:]]))
    return val_acc


cd_work()
layers = np.array([l for l in range(2, 9)])
# cropnet5216 = np.array([load_val_acc('cropnet', (52, 52), (13, 13), l, 16) for l in layers])
...
refnet52323 = np.array([load_val_acc('refnet', (52, 52), (3, 3), l, 32, False) for l in layers])
refnet52643 = np.array([load_val_acc('refnet', (52, 52), (3, 3), l, 64, False) for l in layers])
