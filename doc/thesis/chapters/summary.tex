...
\ac{MAV}s are an emerging technology that supports a wide range of applications. The robust estimation of an \ac{MAV}'s state within its environment is crucial to ensure safe operation.\todo{maybe give one example of application and why the estimation is important } In  indoor scenarios, cameras are one of the predominant choices for state estimation sensors. This requires Computer Vision algorithms to interpret the obtained high dimensional signal. An application that allows the competitive evaluation of control and state estimation algorithms is \ac{MAV} Racing such as the \ac{IROS} 2018 Autonomous Drone Race. Thereby a race court consisting of several race gates has to be followed. For a fast flight during such a race court the detection of the racing gates with a camera can be used in a high level control loop. As these objects consist only of small structures that are spread across large parts of the image, this gives rise to a challenging Object Detection problem.

In recent years \acp{CNN} showed promising results on various vision tasks \todo{maybe mention again that it is for objects different from your gates}. However, due to their computational complexity the deployment on mobile devices remains a challenge. Furthermore, \acp{CNN} typically require a vast amount of training data. This work defines the class of \acp{EWFO} and studies their detection on \acp{MAV} with \ac{Yolo}V3. For the training, data is generated with a graphical engine. \todo{make the research questions appear slightly more clearly} 

\todo{you hsould mention briefly what kind of experiments you did}
...
\todo{add a last sentence about what is the use in general / impact of your thesis for future stuff}
