...
class GateNetEncoder(Encoder):
    ...
    def _assign_true_boxes(self, anchors, true_boxes):
        ...
        for b in true_boxes:
            ...
            for i, b_anchor in enumerate(anchor_boxes):
                iou = b.iou(b_anchor)
                if iou > max_iou and np.isnan(anchors_assigned[i, 4]):
                    max_iou = iou
                    match_idx = i
    ...
    def _encode_coords_level(self, anchors_assigned, level):
        anchors_encoded = anchors_assigned.copy()
        # TODO clean this up
        offset_y, offset_x = np.mgrid[:self.grids[level][0], :self.grids[level][1]]
        offset_x = np.expand_dims(offset_x, -1)
        offset_y = np.expand_dims(offset_y, -1)
        anchors_encoded[:, :2] /= np.ceil(np.array(self.norm) / np.array(self.grids[level]))
        anchors_encoded[:, 2:4] /= np.ceil(np.array(self.norm) / np.array(self.grids[level]))

        anchors_encoded = np.reshape(anchors_encoded,
                                     (self.grids[level][0], self.grids[level][1], -1, self.n_polygon + 1))
        anchors_encoded[:, :, :, 0] -= offset_x
        anchors_encoded[:, :, :, 1] -= offset_y
        anchors_encoded = np.reshape(anchors_encoded, (-1, self.n_polygon + 1))

        return anchors_encoded

    def _encode_coords(self, anchor_assigned):
        coords = []
        for i in range(len(self.grids)):
            coords.append(self._encode_coords_level(anchor_assigned, i))
    ...
    def encode_label(self, label: ImgLabel):
        ...
        label_t = self._encode_coords(label_t)
