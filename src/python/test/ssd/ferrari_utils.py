...
def iou(boxes1, boxes2, coords='centroids'):
    ...
    intersection = np.maximum(0, np.minimum(boxes1[:, 1], boxes2[:, 1]) - np.maximum(boxes1[:, 0],
                                                                                     boxes2[:, 0])) * np.maximum(0,
                                                                                                                 np.minimum(
                                                                                                                     boxes1[
                                                                                                                     :,
                                                                                                                     3],
...
class SSDBoxEncoder:
    ...
    def __init__(self,
        ...
        if scales:
            if (len(scales) != len(
                    predictor_sizes) + 1):  # Must be two nested `if` statements since `list` and `bool` cannot be combined by `&`
                raise ValueError(
        ...
        for i in range(y_encode_template.shape[0]):  # For each batch item...
            ...
            for true_box in ground_truth_labels[i]:  # For each ground truth box belonging to the current batch item...
                ...
                if abs(true_box[2] - true_box[1] < 0.001) or abs(true_box[4] - true_box[
                    3] < 0.001): continue  # Protect ourselves against bad ground truth data: boxes with width or height equal to zero
                if self.normalize_coords:
