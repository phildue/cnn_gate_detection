...
\begin{enumerate}
	...
	\item[\textbf{RQ2}]How does the shape of \acp{EWFO} affect the detection with a \acp{CNN}?
	\item[\textbf{RQ3}]What kind of architecture is suitable to detect \acp{EWFO} on \acp{MAV}?
\end{enumerate}

\section{Outline}


The thesis is structured as follows. \Cref{sec:background} gives an introduction to Object Detection and Synthesizing Data. Also, the target system of this work is illustrated.  \Cref{sec:method} introduces the methodology including the tool used to generate data, the object detection network as well as the test sets. \Cref{sec:experiments} describes the experiments conducted in order to answer the research questions. \Cref{sec:conclusion} discusses the overall results, formulates a conclusion and describes possible future work.
